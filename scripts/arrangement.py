...
class Arrangement:
    ...
    def save(self, file, minN=0):
        ...
        for attr in ["N", "x", "y", "z", "rotY", "rotZ"]:
            if not hasattr(self, attr):
                missing_attrs.append(attr)
        if missing_attrs:
            raise AttributeError(
                f"The following attributes are missing in the arrangement: {', '.join(missing_attrs)}"
            )
        data = np.vstack(
            [
                np.arange(minN, self.N + minN),  # pyright: ignore[reportAttributeAccessIssue]
                self.x,  # pyright: ignore[reportAttributeAccessIssue]
                self.y,  # pyright: ignore[reportAttributeAccessIssue]
                self.z,  # pyright: ignore[reportAttributeAccessIssue]
                self.rotY,  # pyright: ignore[reportAttributeAccessIssue]
                self.rotZ,  # pyright: ignore[reportAttributeAccessIssue]
            ]
        ).T
        ...
        np.savetxt(file, data, fmt=fmt, delimiter=" ", comments="", header=f"{self.N}")  # pyright: ignore[reportAttributeAccessIssue]
...
class PMTArrangement(Arrangement):
    ...
    def __init__(self, detr, xeh, top=True) -> None:
        ...
        self.pmth = (
            WindowThickness + CasingHeight + BaseThickness + PMTToPMTBase
        ) / 10  # in cm
        self.detr = detr
        self.top = top
    ...
    def get_lrtb(self):
        self.xreal = self.x + (self.pmts - self.pmtw) / 2 * np.sqrt(2) * np.cos(
            np.pi / 4 + self.angle / 180 * np.pi
        )
        self.yreal = self.y + (self.pmts - self.pmtw) / 2 * np.sqrt(2) * np.sin(
            np.pi / 4 + self.angle / 180 * np.pi
        )

        self.xcenter = self.x + self.pmts / 2 * np.sqrt(2) * np.cos(
            np.pi / 4 + self.angle / 180 * np.pi
        )
        self.ycenter = self.y + self.pmts / 2 * np.sqrt(2) * np.sin(
            np.pi / 4 + self.angle / 180 * np.pi
        )
        ...
        rbot = np.array(
            [
                self.xreal + self.pmtw * np.cos(self.angle / 180 * np.pi),
                self.yreal + self.pmtw * np.sin(self.angle / 180 * np.pi),
            ]
        )
        rtop = np.array(
            [
                self.xreal + self.pmtw * np.cos((self.angle + 45) / 180 * np.pi),
                self.yreal + self.pmtw * np.sin((self.angle + 45) / 180 * np.pi),
            ]
        )
        ltop = np.array(
            [
                self.xreal + self.pmtw * np.cos((self.angle + 90) / 180 * np.pi),
                self.yreal + self.pmtw * np.sin((self.angle + 90) / 180 * np.pi),
            ]
        )
    ...
    def rectangular(self):
        ...
        lbot = np.array(
            [
                self.xcenter - self.offset + (self.pmts - self.pmtw) / 2,
                self.ycenter - self.offset + (self.pmts - self.pmtw) / 2,
            ]
        )
        rbot = np.array(
            [
    ...
    def get_index(self):
        ...
        for i in range(len(self.lrtb)):
            if np.all(
                np.power(self.lrtb[i] - self.detcenter, 2).sum(axis=1) < self.detr**2
            ):
        ...
        for ii in range(self.lrtb.shape[0]):
            for jj in range(self.lrtb.shape[1]):
                ...
                for i, j in zip([0, 1, 2, 3], [1, 2, 3, 0]):
                    l0 = sub[:, i, :]
                    l1 = sub[:, j, :]
                    cos = (l0 * l1).sum(axis=1) / np.sqrt(
                        (l0**2).sum(axis=1) * (l1**2).sum(axis=1)
                    )
                    accum += np.arccos(cos)
    ...
    def draw(self, file=None):
        ...
        ax.add_patch(
            Circle(
                xy=[0, 0], radius=self.detr, alpha=1.0, facecolor="none", edgecolor="r"
            )
        )
        for i in self.pmtindex:
            ax.add_patch(
                Rectangle(
                    xy=[float(self.xreal[i]), float(self.yreal[i])],
                    width=self.pmtw,
                    height=self.pmtw,
...
class TankArrangement(Arrangement):
    ...
    def getTankOutline(self, radius_ratio=1 / 8, angle=np.pi * 2 / 5, spline_points=11):
        ...
        x.append(
            np.cos(self.small_angle) * (self.small_radius + thickness)
            + (1 - self.radius_ratio) * self.radius
        )
        y.append(np.sin(self.small_angle) * (self.small_radius + thickness))
        large_radius = (1 - self.radius_ratio) * self.radius / np.cos(
            angle
        ) + self.radius_ratio * self.radius
        large_angle = np.linspace(angle, np.pi / 2, spline_points)
        x.append(np.cos(large_angle) * (large_radius + thickness))
        y.append(
            np.sin(large_angle) * (large_radius + thickness)
            - (1 - self.radius_ratio) * self.radius * np.tan(angle)
        )
        x = np.hstack(x)
        y = np.hstack(y)
        # top to bottom
    ...
    def draw(self, file=None):
        ...
        ax.plot(
            [
                0,
                np.cos(self.small_angle[-1]) * self.small_radius
                + (1 - self.radius_ratio) * self.radius,
            ],
            [0, np.sin(self.small_angle[-1]) * self.small_radius + self.height / 2],
            color="k",
            linestyle="dashed",
        )
        ax.plot(
            [(1 - self.radius_ratio) * self.radius, self.radius],
            [self.height / 2, self.height / 2],
            color="k",
            linestyle="dashed",
        )
        ax.scatter(
            [0, (1 - self.radius_ratio) * self.radius],
            [0, self.height / 2],
            color="r",
            s=4.0,
        )
    ...
    def save(
        ...
        if flange_height is not None:
            x = np.hstack(
                [
                    x[: len(x) // 2],
                    [x.max(), x.max() + flange_width, x.max() + flange_width, x.max()],
                    x[len(x) // 2 :],
                ]
            )
            y = np.hstack(
        ...
        np.savetxt(file, data, fmt=fmt, delimiter=" ", comments="", header=f"{len(x)}")
