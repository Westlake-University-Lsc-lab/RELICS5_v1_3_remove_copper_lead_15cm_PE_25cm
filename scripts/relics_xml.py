...
assert radius_outer_gxe >= 0, f"参数 radius_outer_gxe = {radius_outer_gxe} 非法，半径不能为负数"
height_outer_gxe = params['height_outer_gxe']
assert height_outer_gxe >= 0, f"参数 height_outer_gxe = {height_outer_gxe} 非法，高度不能为负数"
shift_z_outer_gxe = params['shift_z_outer_gxe']  # 位移可正可负，不校验

radius_outer_lxe = params['radius_outer_lxe']
assert radius_outer_lxe >= 0, f"参数 radius_outer_lxe = {radius_outer_lxe} 非法，半径不能为负数"
height_outer_lxe = params['height_outer_lxe']
assert height_outer_lxe >= 0, f"参数 height_outer_lxe = {height_outer_lxe} 非法，高度不能为负数"
...
assert radius_teflon_gas >= 0, f"参数 radius_teflon_gas = {radius_teflon_gas} 非法，半径不能为负数"
height_teflon_gas = params['height_teflon_gas']
assert height_teflon_gas >= 0, f"参数 height_teflon_gas = {height_teflon_gas} 非法，高度不能为负数"
shift_z_teflon_gas = params['shift_z_teflon_gas']  # 位移可正可负，不校验

radius_top_teflon = params['radius_top_teflon']
assert radius_top_teflon >= 0, f"参数 radius_top_teflon = {radius_top_teflon} 非法，半径不能为负数"
height_top_teflon = params['height_top_teflon']
assert height_top_teflon >= 0, f"参数 height_top_teflon = {height_top_teflon} 非法，高度不能为负数"
width_x_top_teflon = params['width_x_top_teflon']
assert width_x_top_teflon >= 0, f"参数 width_x_top_teflon = {width_x_top_teflon} 非法，宽度不能为负数"
shift_z_top_teflon = params['shift_z_top_teflon']  # 位移可正可负，不校验

radius_bot_teflon = params['radius_bot_teflon']
assert radius_bot_teflon >= 0, f"参数 radius_bot_teflon = {radius_bot_teflon} 非法，半径不能为负数"
height_bot_teflon = params['height_bot_teflon']
assert height_bot_teflon >= 0, f"参数 height_bot_teflon = {height_bot_teflon} 非法，高度不能为负数"
width_x_bot_teflon = params['width_x_bot_teflon']
assert width_x_bot_teflon >= 0, f"参数 width_x_bot_teflon = {width_x_bot_teflon} 非法，宽度不能为负数"
shift_z_bot_teflon = params['shift_z_bot_teflon']  # 位移可正可负，不校验

# 电极参数
radius_electrode = params['radius_electrode']
assert radius_electrode >= 0, f"参数 radius_electrode = {radius_electrode} 非法，电极半径不能为负数"
wire_diameter = params['wire_diameter']
assert wire_diameter >= 0, f"参数 wire_diameter = {wire_diameter} 非法，导线直径不能为负数"
shift_z_anode = params['shift_z_anode']  # 位移可正可负，不校验
wire_pitch_anode = params['wire_pitch_anode']
assert wire_pitch_anode >= 0, f"参数 wire_pitch_anode = {wire_pitch_anode} 非法，导线间距不能为负数"
shift_z_gate = params['shift_z_gate']  # 位移可正可负，不校验
wire_pitch_gate = params['wire_pitch_gate']
assert wire_pitch_gate >= 0, f"参数 wire_pitch_gate = {wire_pitch_gate} 非法，导线间距不能为负数"
shift_z_cathode = params['shift_z_cathode']  # 位移可正可负，不校验
wire_pitch_cathode = params['wire_pitch_cathode']
assert wire_pitch_cathode >= 0, f"参数 wire_pitch_cathode = {wire_pitch_cathode} 非法，导线间距不能为负数"
...
assert wire_pitch_bottom_screening >= 0, f"参数 wire_pitch_bottom_screening = {wire_pitch_bottom_screening} 非法，导线间距不能为负数"

# 屏蔽参数
lead_width_x = params['lead_width_x']
assert lead_width_x >= 0, f"参数 lead_width_x = {lead_width_x} 非法，铅屏蔽X方向宽度不能为负数"
lead_width_y = params['lead_width_y']
assert lead_width_y >= 0, f"参数 lead_width_y = {lead_width_y} 非法，铅屏蔽Y方向宽度不能为负数"
lead_width_z = params['lead_width_z']
assert lead_width_z >= 0, f"参数 lead_width_z = {lead_width_z} 非法，铅屏蔽Z方向宽度不能为负数"

copper_width_x = params['copper_width_x']
assert copper_width_x >= 0, f"参数 copper_width_x = {copper_width_x} 非法，铜屏蔽X方向宽度不能为负数"
copper_width_y = params['copper_width_y']
assert copper_width_y >= 0, f"参数 copper_width_y = {copper_width_y} 非法，铜屏蔽Y方向宽度不能为负数"
copper_width_z = params['copper_width_z']
assert copper_width_z >= 0, f"参数 copper_width_z = {copper_width_z} 非法，铜屏蔽Z方向宽度不能为负数"

inner_pe_width_x = params['inner_pe_width_x']
assert inner_pe_width_x >= 0, f"参数 inner_pe_width_x = {inner_pe_width_x} 非法，内层PE屏蔽X方向宽度不能为负数"
inner_pe_width_y = params['inner_pe_width_y']
assert inner_pe_width_y >= 0, f"参数 inner_pe_width_y = {inner_pe_width_y} 非法，内层PE屏蔽Y方向宽度不能为负数"
inner_pe_width_z = params['inner_pe_width_z']
assert inner_pe_width_z >= 0, f"参数 inner_pe_width_z = {inner_pe_width_z} 非法，内层PE屏蔽Z方向宽度不能为负数"

# 真空参数
radius_vacuum = params['radius_vacuum']
assert radius_vacuum >= 0, f"参数 radius_vacuum = {radius_vacuum} 非法，真空层半径不能为负数"
height_vacuum = params['height_vacuum']
assert height_vacuum >= 0, f"参数 height_vacuum = {height_vacuum} 非法，真空层高度不能为负数"
shift_z_vacuum = params['shift_z_vacuum']  # 位移可正可负，不校验

# 成形环参数
outer_radius_shapingrings = params['outer_radius_shapingrings']
assert outer_radius_shapingrings >= 0, f"参数 outer_radius_shapingrings = {outer_radius_shapingrings} 非法，成形环外半径不能为负数"
inner_radius_shapingrings = params['inner_radius_shapingrings']
assert inner_radius_shapingrings >= 0, f"参数 inner_radius_shapingrings = {inner_radius_shapingrings} 非法，成形环内半径不能为负数"
...
def format_parameter_value(param_name, value):
    # 不需要单位的参数列表
    no_unit_params = {
        'material', 'type', 'window_partner', 'objects', 'sendname', 
        'base_type', 'casing_finish', 'casing_type', 'window_finish',
        'photocathode_finish', 'source', 'surface', 'check_overlap',
        'opticalsd', 'entrack', 'window_order'
    }
...
# World探测器
create_detector_node(geometry_node, "World", "World", "", {
    "half_x": "500*cm",
    "half_y": "500*cm",
    "half_z": "500*cm"
})
...
# Lead探测器
create_detector_node(geometry_node, "Cuboid", "Lead", "World", {
    "soliname": "LeadSolid",
    "logivol": "LeadLog",
    "physvol": "Lead",
...
# InnerPE探测器
create_detector_node(geometry_node, "Cuboid", "InnerPE", "Lead", {
...
# Air探测器
create_detector_node(geometry_node, "Cuboid", "Air", "InnerPE", {
    "soliname": "AirSolid",
    "logivol": "AirLog",
    "physvol": "Air",
...
# OuterContainer探测器
create_detector_node(geometry_node, "Cylinder", "OuterContainer", "Air", {
...
create_detector_node(geometry_node, "Cuboid", "OuterContainerTop", "Air", {
...
create_detector_node(geometry_node, "Cylinder", "OuterContainerBottom", "Air", {
...
# Pipe探测器
create_detector_node(geometry_node, "Array", "Pipe", "Air", {
    "soliname": "PipeSolid",
    "logivol": "PipeLog",
    "physvol": "Pipe",
    "objects": "Tube",
...
# Flange探测器
create_detector_node(geometry_node, "Array", "Flange", "Air", {
...
# Vacuum探测器
create_detector_node(geometry_node, "Cylinder", "Vacuum", "OuterContainer", {
...
create_detector_node(geometry_node, "Cylinder", "InnerContainerTop", "Vacuum", {
...
create_detector_node(geometry_node, "Cylinder", "InnerContainerBottom", "Vacuum", {
...
# InnerContainer探测器
create_detector_node(geometry_node, "Cylinder", "InnerContainer", "Vacuum", {
...
# OuterGXe探测器
create_detector_node(geometry_node, "Cylinder", "OuterGXe", "InnerContainer", {
...
# OuterLXe探测器
create_detector_node(geometry_node, "Cylinder", "OuterLXe", "InnerContainer", {
...
# tR8520探测器
create_detector_node(geometry_node, "Array", "tR8520", "OuterGXe", {
    "soliname": "R8520Solid",
    "logivol": "R8520Log",
    "physvol": "R8520",
    "objects": "PMTsR8520",
...
# bR8520探测器
create_detector_node(geometry_node, "Array", "bR8520", "OuterLXe", {
    "soliname": "R8520Solid",
    "logivol": "R8520Log",
    "physvol": "R8520",
    "objects": "PMTsR8520",
...
# Teflon探测器
create_detector_node(geometry_node, "Cylinder", "Teflon", "OuterLXe", {
...
# Shapingrings探测器
create_detector_node(geometry_node, "Array", "Shapingrings", "OuterLXe", {
...
# l xenon探测器
create_detector_node(geometry_node, "Cylinder", "lxenon", "Teflon", {
...
# TeflonGas探测器
create_detector_node(geometry_node, "Cylinder", "TeflonGas", "OuterGXe", {
...
# gasxenon探测器
create_detector_node(geometry_node, "Cylinder", "gasxenon", "TeflonGas", {
...
# TopTeflon探测器
create_detector_node(geometry_node, "HolesBoard", "TopTeflon", "gasxenon", {
...
# BotTeflon探测器
create_detector_node(geometry_node, "HolesBoard", "BotTeflon", "lxenon", {
...
# Anode探测器
create_detector_node(geometry_node, "MeshGrid", "Anode", "gasxenon", {
    "soliname": "AnodeSolid",
    "logivol": "AnodeLog",
    "physvol": "Anode",
...
# Gate探测器
create_detector_node(geometry_node, "MeshGrid", "Gate", "lxenon", {
    "soliname": "GateSolid",
    "logivol": "GateLog",
    "physvol": "Gate",
    "radius": f"{radius_electrode}*mm",
...
# Cathode探测器
create_detector_node(geometry_node, "MeshGrid", "Cathode", "lxenon", {
...
# BottomScreening探测器
create_detector_node(geometry_node, "MeshGrid", "BottomScreening", "lxenon", {
...
if save_txt:
    ...
    # 生成成形环位置文件
    tank_arr = TankArrangement(outer_radius_shapingrings, inner_radius_shapingrings,
                              height_shapingrings, number_shapingrings, pitch_shapingrings)
