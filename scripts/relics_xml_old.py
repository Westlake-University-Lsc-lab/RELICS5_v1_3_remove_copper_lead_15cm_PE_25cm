...
for d in detectorList:
    ...
    for x in d['parameters'].keys():
        ...
        if isinstance(d['parameters'][x], str):
            ...
            if x in ['radius', 'height', 'width_x', 'width_y', 'width_z', 
                     'outer_radius', 'inner_radius', 'shift_z', 'wire_diameter', 'wire_pitch']:
                parameter.setAttribute('value', f"{d['parameters'][x]}*mm")
                djson['geometry']['detectors'][-1]['parameters'].update({f'{x}': f"{d['parameters'][x]}*mm"})
            else:
                parameter.setAttribute('value', f"{d['parameters'][x]}")
                djson['geometry']['detectors'][-1]['parameters'].update({f'{x}': f"{d['parameters'][x]}"})
