...
{
  if (parameters.find(name) != parameters.end()) return parameters.at(name);
  return "";
}

double BambooParameters::evaluateParameter(const std::string& name) const
{
  if (parameters.find(name) != parameters.end()) return BambooUtils::evaluate(parameters.at(name));
  return 0;
}
...
void BambooControl::setup(int argc, char* argv[])
{
  ...
  while (true)
  {
    ...
    switch (option)
    {
      ...
      case 'x':
        ...
        if (!loadConfig(config_file_name)) exit(1);
        break;
      case 'i':
        interactive = true;
        break;
  ...
  {
    std::cerr << "A config file must be provided! Try '-c config_file'." << std::endl;
    exit(1);
  }
...
{
  auto parameter =
    std::make_pair(extract_attributes(reader, "name"), extract_attributes(reader, "value"));
  return parameter;
}
...
DetectorInfoTuple BambooControl::read_detector_yaml(const YAML::Node& node)
{
  ...
  if (node["type"]) type = node["type"].as<std::string>();
...
bool BambooControl::loadYamlConfig(const std::string& config_name)
{
  try
  {
    ...
    if (config["physics"])
    {
      ...
      {
        read_pars_yaml(config["physics"]["parameters"], physicsParameters);
      }
    }
    if (config["generator"])
    {
      ...
      {
        read_pars_yaml(config["generator"]["parameters"], generatorParameters);
      }
    }
    if (config["analysis"])
    {
      ...
      {
        read_pars_yaml(config["analysis"]["parameters"], analysisParameters);
      }
    }
    return true;
  }
...
bool BambooControl::sortDetectors(std::set<DetectorInfoTuple>& ds)
{
  ...
  while (!ds.empty())
  {
    auto it = std::find_if(ds.cbegin(), ds.cend(),
                           [&names](const auto& t) { return std::get<2>(t) == names.back(); });
    if (it == ds.cend())
    {
      if (names.back() == "") break;
      names.pop_back();
...
void BambooControl::print() const
{
  ...
  {
    std::cout << "global geometry parameter: " << par.first << " = " << par.second << std::endl;
  }
  ...
  for (const auto& di : detectorInfo)
  {
    ...
    if (detectorParameterMaps.find(name) != detectorParameterMaps.end())
    {
      ...
      {
        std::cout << " -- parameter: " << par.first << " = " << par.second << std::endl;
      }
    }
  }
  ...
  {
    std::cout << "generator parameter: " << par.first << " = " << par.second << std::endl;
  }
...
{
  return new BambooDetectorConstruction{materialName, materialParameters, geometryParameters,
                                        detectorInfo, detectorParameterMaps};
}
...
G4VUserPhysicsList* BambooControl::createPhysics()
{
  if (PhysicsFactory::exists(physicsName))
  {
    ...
    if (physicsList.get() == nullptr) return nullptr;
