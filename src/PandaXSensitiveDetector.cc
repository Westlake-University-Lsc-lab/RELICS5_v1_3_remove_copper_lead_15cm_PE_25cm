...
{
  eDHitsCollection =
    new PandaXEnergyDepositionHitsCollection(SensitiveDetectorName, collectionName[0]);
  hc->AddHitsCollection(GetCollectionID(0), eDHitsCollection);
  fsFluxHitsCollection =
    new PandaXSurfaceFluxHitsCollection(SensitiveDetectorName, collectionName[1]);
...
G4bool PandaXSensitiveDetector::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
  ...
  if (recordEnergyDeposition)
  {
    ...
    if (edep != 0.)
    {
      auto hit = new PandaXEnergyDepositionHit();
  ...
  if (recordSurfaceFlux)
  {
    ...
    if (preStep->GetStepStatus() == fGeomBoundary)  // the first step in the volume
    {
      auto hit = new PandaXSurfaceFluxHit();
