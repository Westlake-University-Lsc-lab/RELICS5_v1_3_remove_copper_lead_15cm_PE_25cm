...
struct SimpleFactory
{
    ...
    static std::unique_ptr<Base> create(Key const& key, Ts... args)
    {
      ...
      if (iter == get_map().end()) return nullptr;
      // call factory function
      return iter->second(args...);
    }

    // check if a give key exist
    static bool exists(Key const& key) { return get_map().find(key) != get_map().end(); }
...
struct Register
{
    Register(Key const& name) { SimpleFactory<Base, Key, Ts...>::get_map()[name] = &create_base; }
