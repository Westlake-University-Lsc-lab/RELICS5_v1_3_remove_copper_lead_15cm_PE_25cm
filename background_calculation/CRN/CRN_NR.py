...
def cosmic_ray_neutron(folder, genNormalize, bins_nr, bins_er, nr_max, nr_min, er_max):
    ...
    # weights = np.full(len(e), genNormalize['CRN']['activity']*days / genNormalize['CRN']['factor']) /  np.diff(bins_er)[np.clip(np.digitize(e['max_er'], bins_er) - 1, 0, len(bins_er) - 2)]/kg
    ...
    cosmic_activity = genNormalize["CRN"]["factor"] / (
    ...
    cosmic_data_nr["CRN"] = cosmic_nr_all
    # cosmic_data_er['CRN']= cosmic_er_all
    ...
    NR_error["CRN"] = nr_error1

    # ER_rate['CRN']=(ER_number_all)*(genNormalize['CRN']['activity']*days / genNormalize['CRN']['factor'])/kg
    # er_error1 = (ER_number_all)**(1/2)*(genNormalize['CRN']['activity']*days / genNormalize['CRN']['factor'])/kg
...
with open(f"./NR_{bins_NR}bins/temp/CRN_data.json", "w") as f:
    f.write(json_str)

json_str = json.dumps(convert_to_list(NR_rate))
with open(f"./NR_{bins_NR}bins/temp/CRN_NR_rate.json", "w") as f:
    f.write(json_str)

json_str = json.dumps(convert_to_list(NR_error))
with open(f"./NR_{bins_NR}bins/temp/CRN_NR_error.json", "w") as f:
    f.write(json_str)
...
with open(f"./NR_{bins_NR}bins/CRN_nr.json", "w") as f:
    f.write(json_str)

json_str = json.dumps(convert_to_list(error))
with open(f"./NR_{bins_NR}bins/CRN_nr_error.json", "w") as f:
    f.write(json_str)

json_str = json.dumps(convert_to_list(bins_nr))
with open(f"./NR_{bins_NR}bins/CRN_nr_bin.json", "w") as f:
    f.write(json_str)
