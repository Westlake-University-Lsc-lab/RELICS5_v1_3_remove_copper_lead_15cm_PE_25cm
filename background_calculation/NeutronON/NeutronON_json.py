...
def CRN_ray_neutron(folder, genNormalize, bins_nr, bins_er, nr_max, nr_min, er_max):
    ...
    files = [os.path.join(folder, "events_neutron_sample_0000001_0000200.h5")]
    with h5py.File(files[0], "r", libver="latest", swmr=True) as ipt:
        events = ipt["events"][:]
        # primaries = ipt['primaries'][:]
    e = events
    # weights = np.full(len(e), genNormalize['neutron_R']['activity']*days / genNormalize['neutron_R']['factor']) / np.diff(bins_nr)[np.clip(np.digitize(e['max_nr'], bins_nr) - 1, 0, len(bins_nr) - 2)]/kg
    ...
    weights = (
        np.full(
            len(e),
            genNormalize["neutron_R"]["activity"]
            * days
            / genNormalize["neutron_R"]["factor"],
        )
        / np.diff(bins_er)[
    ...
    # CRN_activity=genNormalize['neutron_R']['factor']/(genNormalize['neutron_R']['activity']*days)
    ...
    bins = bins_er
    bins_diff = np.diff(bins)
    CRN_activity = genNormalize["neutron_R"]["factor"] / (
        genNormalize["neutron_R"]["activity"] * days
    )
    ...
    CRN_data_er["neutron_R"] = CRN_er_all

    NR_number_all, ER_number_all = rate_error(e, nr_max, nr_min, er_max)

    # NR_rate=(NR_number_all)*(genNormalize['neutron_R']['activity']*days / genNormalize['neutron_R']['factor'])/kg
    # nr_error1 = (NR_number_all)**(1/2)*(genNormalize['neutron_R']['activity']*days / genNormalize['neutron_R']['factor'])/kg
    # NR_error= nr_error1
...
folder = "/root/RELICS5_v1_3_remove_copper_lead_15cm_PE_25cm/result/SampleON_1G_1/"
...
with open(f"./{ER_MIN}_{ER_MAX}keV_{bins_ER}bins/temp/NeutronON_data.json", "w") as f:
    f.write(json_str)

json_str = json.dumps(convert_to_list(ER_rate))
with open(
    f"./{ER_MIN}_{ER_MAX}keV_{bins_ER}bins/temp/NeutronON_ER_rate.json", "w"
) as f:
    f.write(json_str)

json_str = json.dumps(convert_to_list(ER_error))
with open(
    f"./{ER_MIN}_{ER_MAX}keV_{bins_ER}bins/temp/NeutronON_ER_error.json", "w"
) as f:
    f.write(json_str)
...
with open(f"./{ER_MIN}_{ER_MAX}keV_{bins_ER}bins/NeutronON_er.json", "w") as f:
    f.write(json_str)

json_str = json.dumps(convert_to_list(error))
with open(f"./{ER_MIN}_{ER_MAX}keV_{bins_ER}bins/NeutronON_er_error.json", "w") as f:
    f.write(json_str)

json_str = json.dumps(convert_to_list(bins_er))
with open(f"./{ER_MIN}_{ER_MAX}keV_{bins_ER}bins/NeutronON_er_bin.json", "w") as f:
    f.write(json_str)
...
# print('NeutronON NR rate:',NR_rate,'/kg/days')
# print('NeutronON NR error:',NR_error)
print("NeutronON ER rate:", CRN_ER / (ER_MAX - ER_MIN), "/kg/days/keV")
print("NeutronON ER error:", CRN_Error / (ER_MAX - ER_MIN))
ER_rate_mDRU = CRN_ER / (ER_MAX - ER_MIN) * 1000
ER_error_mDRU = CRN_Error / (ER_MAX - ER_MIN) * 1000
print("NeutronON ER rate(mDRU):", ER_rate_mDRU)
print("NeutronON ER error(mDRU):", ER_error_mDRU)
