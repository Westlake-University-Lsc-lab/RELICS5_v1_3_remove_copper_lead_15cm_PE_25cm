...
def cosmic_ray_neutron(folder, genNormalize, bins_nr, bins_er, nr_max, nr_min, er_max):
    ...
    # weights = np.full(len(e), genNormalize['neutron_R']['activity']*days / genNormalize['neutron_R']['factor']) /  np.diff(bins_er)[np.clip(np.digitize(e['max_er'], bins_er) - 1, 0, len(bins_er) - 2)]/kg
    ...
    cosmic_activity = genNormalize["neutron_R"]["factor"] / (
    ...
    cosmic_data_nr["neutron_R"] = cosmic_nr_all
    # cosmic_data_er['neutron_R']= cosmic_er_all
    ...
    NR_error["neutron_R"] = nr_error1

    # ER_rate['neutron_R']=(ER_number_all)*(genNormalize['neutron_R']['activity']*days / genNormalize['neutron_R']['factor'])/kg
    # er_error1 = (ER_number_all)**(1/2)*(genNormalize['neutron_R']['activity']*days / genNormalize['neutron_R']['factor'])/kg
...
with open(f"./NR_{bins_NR}bins/temp/neutron_R_data.json", "w") as f:
    f.write(json_str)

json_str = json.dumps(convert_to_list(NR_rate))
with open(f"./NR_{bins_NR}bins/temp/neutron_R_NR_rate.json", "w") as f:
    f.write(json_str)

json_str = json.dumps(convert_to_list(NR_error))
with open(f"./NR_{bins_NR}bins/temp/neutron_R_NR_error.json", "w") as f:
    f.write(json_str)
...
with open(f"./NR_{bins_NR}bins/neutron_R_nr.json", "w") as f:
    f.write(json_str)

json_str = json.dumps(convert_to_list(error))
with open(f"./NR_{bins_NR}bins/neutron_R_nr_error.json", "w") as f:
    f.write(json_str)

json_str = json.dumps(convert_to_list(bins_nr))
with open(f"./NR_{bins_NR}bins/neutron_R_nr_bin.json", "w") as f:
    f.write(json_str)
...
print("neutron_R NR rate:", NR_rate, "/kg/days")
print("neutron_R NR error:", NR_error)
