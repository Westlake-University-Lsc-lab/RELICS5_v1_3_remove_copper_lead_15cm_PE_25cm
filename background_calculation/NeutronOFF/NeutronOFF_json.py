...
def CRN_ray_neutron(folder, genNormalize, bins_nr, bins_er, nr_max, nr_min, er_max):
    ...
    e = events
    # weights = np.full(len(e), genNormalize['neutron_E']['activity']*days / genNormalize['neutron_E']['factor']) / np.diff(bins_nr)[np.clip(np.digitize(e['max_nr'], bins_nr) - 1, 0, len(bins_nr) - 2)]/kg
    ...
    # CRN_activity=genNormalize['neutron_E']['factor']/(genNormalize['neutron_E']['activity']*days)
    ...
    bins = bins_er
    bins_diff = np.diff(bins)
    CRN_activity = genNormalize["neutron_E"]["factor"] / (
    ...
    CRN_data_er["neutron_E"] = CRN_er_all

    NR_number_all, ER_number_all = rate_error(e, nr_max, nr_min, er_max)

    # NR_rate=(NR_number_all)*(genNormalize['neutron_E']['activity']*days / genNormalize['neutron_E']['factor'])/kg
    # nr_error1 = (NR_number_all)**(1/2)*(genNormalize['neutron_E']['activity']*days / genNormalize['neutron_E']['factor'])/kg
    # NR_error= nr_error1
...
with open(f"./{ER_MIN}_{ER_MAX}keV_{bins_ER}bins/temp/NeutronOFF_data.json", "w") as f:
    f.write(json_str)

json_str = json.dumps(convert_to_list(ER_rate))
with open(
    f"./{ER_MIN}_{ER_MAX}keV_{bins_ER}bins/temp/NeutronOFF_ER_rate.json", "w"
) as f:
    f.write(json_str)

json_str = json.dumps(convert_to_list(ER_error))
with open(
    f"./{ER_MIN}_{ER_MAX}keV_{bins_ER}bins/temp/NeutronOFF_ER_error.json", "w"
) as f:
    f.write(json_str)
...
with open(f"./{ER_MIN}_{ER_MAX}keV_{bins_ER}bins/NeutronOFF_er.json", "w") as f:
    f.write(json_str)

json_str = json.dumps(convert_to_list(error))
with open(f"./{ER_MIN}_{ER_MAX}keV_{bins_ER}bins/NeutronOFF_er_error.json", "w") as f:
    f.write(json_str)

json_str = json.dumps(convert_to_list(bins_er))
with open(f"./{ER_MIN}_{ER_MAX}keV_{bins_ER}bins/NeutronOFF_er_bin.json", "w") as f:
    f.write(json_str)
...
# print('NeutronOFF NR rate:',NR_rate,'/kg/days')
# print('NeutronOFF NR error:',NR_error)
print("NeutronOFF ER rate:", CRN_ER / (ER_MAX - ER_MIN), "/kg/days/keV")
print("NeutronOFF ER error:", CRN_Error / (ER_MAX - ER_MIN))
ER_rate_mDRU = CRN_ER / (ER_MAX - ER_MIN) * 1000
ER_error_mDRU = CRN_Error / (ER_MAX - ER_MIN) * 1000
print("NeutronOFF ER rate(mDRU):", ER_rate_mDRU)
print("NeutronOFF ER error(mDRU):", ER_error_mDRU)
