...
dis_cathode = 30
dis_botscreen = 3
dis_fv_top_to_lxe = 1.835  # 单位cm
semi_axis_c = 14.2325  # 单位cm
fv_r = 136.46  # 主探测器中的半径，单位mm
fv_z1 = (
...
def muon(folder, genNormalize, bins_nr, bins_er, nr_max, nr_min, er_max):
    ...
    files = [os.path.join(folder, "events_muon.h5")]
    ...
    weights = (
        np.full(
            len(e),
            genNormalize["muon"]["activity"] * days / genNormalize["muon"]["factor"],
        )
        / np.diff(bins_nr)[
    ...
    h = np.histogram(
        e["max_nr"][cut_fv(e, "nr") & cut_nr_er(e) & cut_nr(e) & muon_veto_nr(e)],
        weights=weights[cut_fv(e, "nr") & cut_nr_er(e) & cut_nr(e) & muon_veto_nr(e)],
        bins=bins_nr,
    )
    muon_nr_veto = h[0]
    ...
    muon_activity = genNormalize["muon"]["factor"] / (
        genNormalize["muon"]["activity"] * days
    )
    c = muon_veto / (muon_activity * kg)
    nr_veto_error = [
        (a * c / b) ** (1 / 2) for a, b in zip(muon_nr_veto * 0.01, bins_diff)
    ]
    ...
    muon_nr_error = [
        (a**2 + b**2) ** (1 / 2) for a, b in zip(nr_noveto_error, nr_veto_error)
    ]
    ...
    NR_rate["muon"] = (NR_number_all - NR_number_veto) * (
        genNormalize["muon"]["activity"] * days / genNormalize["muon"]["factor"]
    ) / kg + 0.01 * NR_number_veto * (
        genNormalize["muon"]["activity"] * days / genNormalize["muon"]["factor"]
    ) / kg
    nr_error1 = (
        (NR_number_all - NR_number_veto) ** (1 / 2)
        * (genNormalize["muon"]["activity"] * days / genNormalize["muon"]["factor"])
        / kg
    )
    nr_error2 = (
    ...
    ER_rate["muon"] = (ER_number_all - ER_number_veto) * (
        genNormalize["muon"]["activity"] * days / genNormalize["muon"]["factor"]
    ) / kg + 0.01 * ER_number_veto * (
        genNormalize["muon"]["activity"] * days / genNormalize["muon"]["factor"]
    ) / kg
    er_error1 = (
        (ER_number_all - ER_number_veto) ** (1 / 2)
        * (genNormalize["muon"]["activity"] * days / genNormalize["muon"]["factor"])
        / kg
    )
    er_error2 = (
...
f = open("../normalization_muon.json", "r")
content = f.read()
genNormalize = json.loads(content)


# the position of muon data
folder = "/public/home/yyy/result_yyy_soild/muon_1000cm_6billion"
bins_nr = np.logspace(-4, 7, 101)
bins_er = np.linspace(0, 20, 21)
nr_max = 1.36
nr_min = 0.63
er_max = 20
(
    ...
    ER_rate,
    ER_error,
) = muon(folder, genNormalize, bins_nr, bins_er, nr_max, nr_min, er_max)
