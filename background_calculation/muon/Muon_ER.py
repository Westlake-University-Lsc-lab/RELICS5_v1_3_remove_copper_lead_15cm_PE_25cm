...
def muon_rate_error(events, nr_max, nr_min, er_max):
    ...
    ER_number_veto = len(
        e["max_er"][cut_fv(e, "er") & cut_er(e) & cut_er_t(e) & muon_veto_er(e) & mask]
    )
...
def muon(folder, genNormalize, bins_nr, bins_er, nr_max, nr_min, er_max):
    ...
    h = np.histogram(
        e["max_er"][cut_fv(e, "er") & cut_er(e) & muon_veto_er(e) & cut_er_t(e)],
        weights=weights[cut_fv(e, "er") & cut_er(e) & muon_veto_er(e) & cut_er_t(e)],
        bins=bins_er,
    )
    muon_er_veto = h[0]
    ...
    c = muon_veto / (muon_activity * kg)
    er_veto_error = [
        (a * c / b) ** (1 / 2) for a, b in zip(muon_er_veto * 0.01, bins_diff)
    ]
    ...
    muon_er_error = [
        (a**2 + b**2) ** (1 / 2) for a, b in zip(er_noveto_error, er_veto_error)
    ]
...
folder = f"/public/home/yyy/result_yyy_soild/6billion_预处理/{ER_MAX}keV"
bins_nr = np.logspace(-4, 7, 101)
nr_max = 1.36
nr_min = 0.63
er_max = float(ER_MAX)
bins_er = np.linspace(0, er_max, bins_ER + 1)
muon_events, muon_data_er, muon_er_error, ER_rate, ER_error = muon(
...
# 新增：将数据写入HDF5文件
folder = f"./{ER_MAX}keV_{bins_ER}bins"
output_file = os.path.join(folder, "muon_results.h5")
...
print("muon ER rate:", Muon_ER / ER_MAX, "/kg/days/keV")
print("muon ER error:", Muon_Error / ER_MAX)
ER_rate_mDRU = Muon_ER / ER_MAX * 1000
ER_error_mDRU = Muon_Error / ER_MAX * 1000
