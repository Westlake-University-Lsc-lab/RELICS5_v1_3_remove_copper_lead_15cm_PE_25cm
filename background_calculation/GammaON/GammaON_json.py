...
def CRN_ray_neutron(folder, genNormalize, bins_nr, bins_er, nr_max, nr_min, er_max):
    ...
    files = [os.path.join(folder, "events_gamma_ON_90B.h5")]
    with h5py.File(files[0], "r", libver="latest", swmr=True) as ipt:
        events = ipt["events"][:]
        # primaries = ipt['primaries'][:]
    e = events
    # weights = np.full(len(e), genNormalize['gamma_R']['activity']*days / genNormalize['gamma_R']['factor']) / np.diff(bins_nr)[np.clip(np.digitize(e['max_nr'], bins_nr) - 1, 0, len(bins_nr) - 2)]/kg
    ...
    weights = (
        np.full(
            len(e),
            genNormalize["gamma_R"]["activity"]
            * days
            / genNormalize["gamma_R"]["factor"],
        )
        / np.diff(bins_er)[
    ...
    # CRN_activity=genNormalize['gamma_R']['factor']/(genNormalize['gamma_R']['activity']*days)
    ...
    bins = bins_er
    bins_diff = np.diff(bins)
    CRN_activity = genNormalize["gamma_R"]["factor"] / (
        genNormalize["gamma_R"]["activity"] * days
    )
    ...
    CRN_data_er["gamma_R"] = CRN_er_all

    NR_number_all, ER_number_all = rate_error(e, nr_max, nr_min, er_max)

    # NR_rate=(NR_number_all)*(genNormalize['gamma_R']['activity']*days / genNormalize['gamma_R']['factor'])/kg
    # nr_error1 = (NR_number_all)**(1/2)*(genNormalize['gamma_R']['activity']*days / genNormalize['gamma_R']['factor'])/kg
    # NR_error= nr_error1
...
with open(f"./{ER_MIN}_{ER_MAX}keV_{bins_ER}bins/temp/GammaON_data.json", "w") as f:
    f.write(json_str)

json_str = json.dumps(convert_to_list(ER_rate))
with open(f"./{ER_MIN}_{ER_MAX}keV_{bins_ER}bins/temp/GammaON_ER_rate.json", "w") as f:
    f.write(json_str)

json_str = json.dumps(convert_to_list(ER_error))
with open(f"./{ER_MIN}_{ER_MAX}keV_{bins_ER}bins/temp/GammaON_ER_error.json", "w") as f:
    f.write(json_str)
...
with open(f"./{ER_MIN}_{ER_MAX}keV_{bins_ER}bins/GammaON_er.json", "w") as f:
    f.write(json_str)

json_str = json.dumps(convert_to_list(error))
with open(f"./{ER_MIN}_{ER_MAX}keV_{bins_ER}bins/GammaON_er_error.json", "w") as f:
    f.write(json_str)

json_str = json.dumps(convert_to_list(bins_er))
with open(f"./{ER_MIN}_{ER_MAX}keV_{bins_ER}bins/GammaON_er_bin.json", "w") as f:
    f.write(json_str)
...
# print('GammaON NR rate:',NR_rate,'/kg/days')
# print('GammaON NR error:',NR_error)
print("GammaON ER rate:", CRN_ER / (ER_MAX - ER_MIN), "/kg/days/keV")
print("GammaON ER error:", CRN_Error / (ER_MAX - ER_MIN))
ER_rate_mDRU = CRN_ER / (ER_MAX - ER_MIN) * 1000
ER_error_mDRU = CRN_Error / (ER_MAX - ER_MIN) * 1000
print("GammaON ER rate(mDRU):", ER_rate_mDRU)
print("GammaON ER error(mDRU):", ER_error_mDRU)
