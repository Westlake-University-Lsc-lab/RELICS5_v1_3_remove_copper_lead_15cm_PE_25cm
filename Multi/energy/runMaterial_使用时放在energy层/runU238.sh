...
for isotope in $isotopes; do
    for component in $components; do
        ...
        jq ".$SUFFIX=8000" $norm | sponge $norm
        ./run.sh -m material -i $isotope -t $component -f $(expr $files \* 8) -j $parallel -e $(expr $events \* 1000) $clean $justprint
    done
done
...
for isotope in $isotopes; do
    for component in $components; do
        export SUFFIX="_${isotope}_${component}"
        jq ".$SUFFIX=20000" $norm | sponge $norm
        ./run.sh -m material -i $isotope -t $component -f $(expr $files \* 20) -j $parallel -e $(expr $events \* 1000) $clean $justprint
    done
done
