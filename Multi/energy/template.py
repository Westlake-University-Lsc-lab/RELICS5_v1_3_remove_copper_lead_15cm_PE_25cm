...
class TemplateGenerate:
    ...
    def __init__(self, folder, norm, veto=1e-2) -> None:
        ...
        self.folder = folder
        self.veto = veto
        assert self.veto >= 0.0 and self.veto <= 1.0
    ...
    def apply_cut(self, key):
        ...
        if key == "Rn222_lXe":
            bad_events = np.isin(
                self.events[key]["primaries"]["primaryType"].values.astype(str),
                self.bad_primaries,
            )
        ...
        data = (
            [cuts[name] for name in self.dtype.names]
            + [max_nr_r2, max_er_r2]
            + [[key] * len(events)]
        )
    ...
    def get_hist(self):
        for key in self.events.keys():
            for e in ["nr", "er"]:
                ...
                self.events[key][e] = np.histogram(
                    event[f"max_{e}"], bins=self.bins[e], weights=weights
                )[0]

                self.events[key][f"{e}_xy"] = np.histogram2d(
                    event[f"max_{e}_x"],
                    event[f"max_{e}_y"],
                ...
                # r = r2 ** 0.5
                self.events[key][f"{e}_rz"] = np.histogram2d(
                    r2,
                    event[f"max_{e}_z"],
    ...
    def big_combine(self):
        ...
        self.combine_primaries = pd.concat(
            [
                self.events[key]["primaries"]
                for key in self.events.keys()
                if "OFF" not in key
            ]
        )
    ...
    def save(self, file):
        ...
        np.savez(
            ...
            * self.mass
            / self.mass_infv
            / np.diff(self.bins["nr"]),
            NRbins=self.bins["nr"],
            ER=self.er_hist
            * 365
            * self.mass
            / self.mass_infv
            / np.diff(self.bins["er"]),
            ERbins=self.bins["er"],
        )
