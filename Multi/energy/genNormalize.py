...
parser = argparse.ArgumentParser(description='Generate normalization factor after simulation')
parser.add_argument('--InputFile', dest='InputFile', required=True, help='Input .json file with simulated events')
parser.add_argument('--paramsFile', dest='paramsFile', required=True, help='Parameters .json file')
parser.add_argument('--GeoFile', dest='GeoFile', required=True, help='Geometry setup .json file')
parser.add_argument('--OutputFile', dest='OutputFile', required=True, help='Output .json file')
...
shutil.copy(GeoFilename, '../analysis/geo_params.json')  # 修正：复制几何文件而非参数文件
...
if 'Shapingrings' in geom_mass:
    ...
    volume_cm3 = np.pi * (outer_r**2 - inner_r**2) * height * num_rings  # 修正：乘以环数量
...
steel_components = ['OuterContainer', 'InnerContainer', 'Flange', 
                   'Anode', 'Cathode', 'Gate', 'BottomScreening', 'Casing']  # 修正：统一Flange命名
steel_mass = sum(geom_mass[comp][0] for comp in steel_components if comp in geom_mass)
steel_dens = geom_mass['OuterContainer'][1] if 'OuterContainer' in geom_mass else 8.00  # 修正：使用Material.cc中SS304LSteel密度
...
teflon_dens = geom_mass['Teflon'][1] if 'Teflon' in geom_mass else 2.2  # 与Material.cc一致
...
normalization['PMTwindow'] = {
    ...
    'volume': (geom_mass['Window'][0] * 1e3 / geom_mass['Window'][1]) if 'Window' in geom_mass else 45.45,
    'num': pmtN
}
...
normalization['PMTbase'] = {
    ...
    'volume': (geom_mass['Base'][0] * 1e3 / geom_mass['Base'][1]) if 'Base' in geom_mass else 25.06,
    'num': pmtN
}
...
lxe_dens = geom_mass['XenonDetector'][1] if 'XenonDetector' in geom_mass else 2.862  # 修正：使用Material.cc中LXe密度
...
# 缪子和CRN归一化
normalization['muon'] = {
    'factor': norm['_muon'] * eventALL,
    'activity': m_rate
}

normalization['CRN'] = {
    'factor': norm['_CRN'] * eventALL,
    'activity': CRN_rate
}
...
# 特殊同位素（PMT窗口铯137）
normalization['Cs137_PMTwindow'] = {
    'factor': norm['_Cs137_PMTwindow'] * eventALL
}
