...
export FOLDER=/public/home/yyy/RELICS5各个版本/RELICS5_v1/RELICS5_v1_3_remove_copper各个版本/RELICS5_v1/RELICS5_v1_1_PE_in/result/R_E_test
reactors="ON OFF"
topsides="TOP SIDE"
for reactor in $reactors; do
    for topside in $topsides; do
        ...
        ./run.sh -m neutron -r $reactor -s $topside -f $(expr $files \* 1) -j $parallel -e $(expr $events \* 1000) $clean $justprint
    done
done
...
for reactor in $reactors; do
    for topside in $topsides; do
        ...
        jq ".$SUFFIX=1000" $norm | sponge $norm
        ./run.sh -m gamma -r $reactor -s $topside -f $(expr $files \* 1) -j $parallel -e $(expr $events \* 1000) $clean $justprint
    done
done