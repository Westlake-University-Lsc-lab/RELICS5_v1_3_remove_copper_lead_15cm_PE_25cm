...
SampleCylinder::SampleCylinder(const std::string& n, const BambooParameters& pars)
  : BambooDetector(n, pars)
{
  G4cout << "create detector SampleCylinder..." << G4endl;
}

bool SampleCylinder::construct(const BambooParameters&, BambooDetector* parent)
{
  ...
  auto detectorSoild = new G4Tubs("detectorSoild", 0, radius, height / 2, 0, 2 * M_PI);
  mainLV = new G4LogicalVolume(detectorSoild, lXe, "SampleCylinderLV", 0, 0, 0);
  mainPV = new G4PVPlacement(0, G4ThreeVector(shift_x, shift_y, shift_z), mainLV, "SampleCylinder",
