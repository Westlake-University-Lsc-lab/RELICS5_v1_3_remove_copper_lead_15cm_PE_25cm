...
SampleSteelContainer::SampleSteelContainer(const std::string& n, const BambooParameters& pars)
  : BambooDetector(n, pars)
{
  G4cout << "create detector SampleSteelContainer..." << G4endl;
}

bool SampleSteelContainer::construct(const BambooParameters&, BambooDetector* parent)
{
  ...
  auto containerSolid = new G4Tubs("containerSoild", 0, radius, height / 2, 0, 2 * M_PI);
  mainLV = new G4LogicalVolume(containerSolid, steel, "ContainerLog", 0, 0, 0);
  mainPV = new G4PVPlacement(0, G4ThreeVector(0, 0, 0), mainLV, "SampleSteelContainer",
