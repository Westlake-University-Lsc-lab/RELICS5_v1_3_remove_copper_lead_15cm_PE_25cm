...
PMTsR8520::PMTsR8520(const std::string&, const BambooParameters& pars) : parameters{pars}
{
...
PMTLVs PMTsR8520::construct(const BambooParameters&, BambooDetector* parent)
{
  ...
  const G4double HalfZ = 0.5 * (WindowThickness + CasingHeight + PMTToPMTBase + BaseThickness);
  ...
  G4Box* BaseBox = new G4Box("BaseBox", BaseHalfX, BaseHalfX, BaseHalfZ);

  BaseLogicalVolume = new G4LogicalVolume(BaseBox, Cirlex, "BaseLogicalVolume", 0, 0, 0);
  ...
  G4Box* WindowBox = new G4Box("WindowBox", WindowHalfX, WindowHalfX, WindowHalfZ);

  WindowLogicalVolume = new G4LogicalVolume(WindowBox, Quartz, "WindowLogicalVolume", 0, 0, 0);
  ...
  G4Box* CasingBox = new G4Box("CasingBox", CasingHalfX, CasingHalfX, CasingHalfZ);

  CasingLogicalVolume = new G4LogicalVolume(CasingBox, SS304LSteel, "CasingLogicalVolume", 0, 0, 0);

  // Interior
  const G4double InteriorHalfX = CasingHalfX - CasingThickness;
  const G4double InteriorHalfZ = CasingHalfZ - CasingThickness;

  G4Box* InteriorBox = new G4Box("InteriorBox", InteriorHalfX, InteriorHalfX, InteriorHalfZ);

  InteriorLogicalVolume =
    new G4LogicalVolume(InteriorBox, Vacuum, "InteriorLogicalVolume", 0, 0, 0);

  // Photocathode
  const G4double PhotoCathodeHalfX = 0.5 * PhotoCathodeWidth;
  const G4double PhotoCathodeHalfZ = 0.5 * PhotoCathodeThickness;
  const G4double PhotoCathodeOffsetZ = PhotoCathodeHalfZ - CasingHalfZ;

  G4Box* PhotoCathodeBox =
    new G4Box("PhotoCathodeBox", PhotoCathodeHalfX, PhotoCathodeHalfX, PhotoCathodeHalfZ);

  PhotoCathodeLogicalVolume = new G4LogicalVolume(PhotoCathodeBox, PhotoCathodeAluminium,
  ...
  BasePhysicalVolume = new G4PVPlacement(0, G4ThreeVector(0., 0., BaseOffset), BaseLogicalVolume,
                                         "Base", CombineLogicalVolume, false, 0, check_overlap);

  WindowPhysicalVolume =
    new G4PVPlacement(0, G4ThreeVector(0., 0., WindowOffset), WindowLogicalVolume, "Window",
                      CombineLogicalVolume, false, 0, check_overlap);

  CasingPhysicalVolume =
    new G4PVPlacement(0, G4ThreeVector(0., 0., CasingOffset), CasingLogicalVolume, "Casing",
                      CombineLogicalVolume, false, 0, check_overlap);

  InteriorPhysicalVolume =
    new G4PVPlacement(0, G4ThreeVector(0., 0., 0.), InteriorLogicalVolume, "Interior",
                      CasingLogicalVolume, false, 0, check_overlap);

  PhotoCathodePhysicalVolume =
    new G4PVPlacement(0, G4ThreeVector(0., 0., PhotoCathodeOffsetZ), PhotoCathodeLogicalVolume,
                      "PhotoCathode", CasingLogicalVolume, false, 0, check_overlap);
  ...
  pMaterial->AddLogicalSurface(PhotoCathodePhysicalVolume, parent->getMainPV(), parameters,
                               "photocathode_");
  ...
  G4VisAttributes* PhotocathodeVisAtt = new G4VisAttributes(hPhotocathodeColor);
