...
void Material::defineMaterials(const BambooParameters& parameters)
{
  ...
  // ==== Liquid xenon ====
  auto LXe = new G4Material("LXe", 2.862 * g / cm3, 1, kStateLiquid, Temperature, Pressure);
  LXe->AddElement(Xe, 1);
  ...
  pLXePropertiesTable->AddProperty("SCINTILLATIONCOMPONENT1", pLXePhotonMomentum, pLXeScintillation,
                                   3);
  pLXePropertiesTable->AddProperty("SCINTILLATIONCOMPONENT2", pLXePhotonMomentum, pLXeScintillation,
                                   3);
...
#else
  pLXePropertiesTable->AddProperty("FASTCOMPONENT", pLXePhotonMomentum, pLXeScintillation, 3);
  pLXePropertiesTable->AddProperty("SLOWCOMPONENT", pLXePhotonMomentum, pLXeScintillation, 3);
  ...
  // ==== Gaseous xenon ====
  auto GXe = new G4Material("GXe", 0.005887 * g / cm3, 1, kStateGas, Temperature, Pressure);
  GXe->AddElement(Xe, 1);
...
#if G4VERSION_NUMBER >= 1100
  pGXePropertiesTable->AddProperty("SCINTILLATIONCOMPONENT1", pGXePhotonMomentum, pGXeScintillation,
                                   3);
  pGXePropertiesTable->AddProperty("SCINTILLATIONCOMPONENT2", pGXePhotonMomentum, pGXeScintillation,
                                   3);
...
#else
  pGXePropertiesTable->AddProperty("FASTCOMPONENT", pGXePhotonMomentum, pGXeScintillation, 3);
  pGXePropertiesTable->AddProperty("SLOWCOMPONENT", pGXePhotonMomentum, pGXeScintillation, 3);
  ...
  pPhotoCathodePropertiesTable->AddProperty("ABSLENGTH", pPhotoCathodePhotonMomentum,
                                            pPhotoCathodeAbsorbtionLength, 5);
  pPhotoCathodePropertiesTable->AddProperty("REFLECTIVITY", pPhotoCathodePhotonMomentum,
                                            pPhotoCathodeReflectivity, 5);
  pPhotoCathodePropertiesTable->AddProperty("EFFICIENCY", pPhotoCathodePhotonMomentum,
                                            pPhotoCathodeEsfficiency, 5);
  pPhotoCathodePropertiesTable->AddProperty("TRANSMITTANCE", pPhotoCathodePhotonMomentum,
                                            pPhotoCathodeTransmittance, 5);
  ...
  Flange->AddElement(Mn, 0.02);
  Flange->AddElement(Si, 0.01);

  G4double pFlangePhotonMomentum[] = {1. * eV, 5. * eV};
  ...
  pFlangePropertiesTable->AddProperty("TRANSMITTANCE", pFlangePhotonMomentum, pFlangeTransmittance,
                                      2);
  SetProperties(parameters, "Flange_", pFlangePropertiesTable);
  Flange->SetMaterialPropertiesTable(pFlangePropertiesTable);
  ...
  pTeflonPropertiesTable->AddProperty("ABSLENGTH", pTeflonPhotonMomentum, pTeflonAbsorbtionLength,
                                      3);
  pTeflonPropertiesTable->AddProperty("REFLECTIVITY", pTeflonPhotonMomentum, pTeflonReflectivity,
                                      3);
  pTeflonPropertiesTable->AddProperty("SPECULARLOBECONSTANT", pTeflonPhotonMomentum,
                                      pTeflonSpecularLobe, 3);
  pTeflonPropertiesTable->AddProperty("SPECULARSPIKECONSTANT", pTeflonPhotonMomentum,
                                      pTeflonSpecularSpike, 3);
  pTeflonPropertiesTable->AddProperty("BACKSCATTERCONSTANT", pTeflonPhotonMomentum,
                                      pTeflonBackscatter, 3);
  pTeflonPropertiesTable->AddProperty("TRANSMITTANCE", pTeflonPhotonMomentum, pTeflonTransmittance,
                                      3);
  ...
  G4double pTeflonGasPhotonMomentum[] = {6.91 * eV, 6.98 * eV, 7.05 * eV};
  G4double pTeflonGasRefractiveIndex[] = {1.63, 1.61, 1.58};
  G4double pTeflonGasAbsorbtionLength[] = {0.1 * cm, 0.1 * cm, 0.1 * cm};
  G4double pTeflonGasReflectivity[] = {0.99, 0.99, 0.99};
  G4double pTeflonGasSpecularLobe[] = {0.01, 0.01, 0.01};
  ...
  pTeflonGasPropertiesTable->AddProperty("ABSLENGTH", pTeflonGasPhotonMomentum,
                                         pTeflonGasAbsorbtionLength, 3);
  pTeflonGasPropertiesTable->AddProperty("REFLECTIVITY", pTeflonGasPhotonMomentum,
                                         pTeflonGasReflectivity, 3);
  pTeflonGasPropertiesTable->AddProperty("SPECULARLOBECONSTANT", pTeflonGasPhotonMomentum,
                                         pTeflonGasSpecularLobe, 3);
  pTeflonGasPropertiesTable->AddProperty("SPECULARSPIKECONSTANT", pTeflonGasPhotonMomentum,
                                         pTeflonGasSpecularSpike, 3);
  pTeflonGasPropertiesTable->AddProperty("BACKSCATTERCONSTANT", pTeflonGasPhotonMomentum,
                                         pTeflonGasBackscatter, 3);
  pTeflonGasPropertiesTable->AddProperty("TRANSMITTANCE", pTeflonGasPhotonMomentum,
                                         pTeflonGasTransmittance, 3);
  SetProperties(parameters, "TeflonGas_", pTeflonGasPropertiesTable);
  TeflonGas->SetMaterialPropertiesTable(pTeflonGasPropertiesTable);
...
// </material>
void Material::SetProperties(const BambooParameters& parameters, const std::string& material,
...
void Material::SetConstProperty(G4MaterialPropertiesTable* table, const std::string& property,
                                G4String newValue)
{
  std::istringstream instring(newValue);
  G4String tmp;
...
G4OpticalSurface* Material::EvalOpParameters(const BambooParameters& parameters,
                                             G4VPhysicalVolume* mainPV, const G4String& prefix)
{
...
void Material::AddLogicalSkinSurface(G4LogicalVolume* mainLV, G4OpticalSurface* opSurface,
                                     const BambooParameters& parameters, const G4String& prefix)
{
  auto material = parameters.getParameter(prefix + "material");
...
void Material::AddLogicalBorderSurface(G4VPhysicalVolume* mainPV, G4VPhysicalVolume* containerPV,
                                       ...
                                       const BambooParameters& parameters, const G4String& prefix)
{
  ...
  new G4LogicalBorderSurface(borderpvs.vol1->GetName() + "_to_" + borderpvs.vol2->GetName()
                               + "_Border",
                             borderpvs.vol1, borderpvs.vol2, opSurface);
  G4cout << "Set G4LogicalBorderSurface from "
              "" + borderpvs.vol1->GetName() +
                  " to "
                  "" +
                  borderpvs.vol2->GetName() + " with " + opSurface->GetName()
           << G4endl;
}
...
void Material::AddLogicalSurface(G4VPhysicalVolume* mainPV, G4VPhysicalVolume* containerPV,
                                 const BambooParameters& parameters, const G4String& prefix)
{
  ...
  if (partner == "")
  {
    ...
    AddLogicalSkinSurface(mainPV->GetLogicalVolume(), opSurface, parameters, prefix);
  }
  else
  {
    AddLogicalBorderSurface(mainPV, containerPV, opSurface, parameters, prefix);
  }
}
