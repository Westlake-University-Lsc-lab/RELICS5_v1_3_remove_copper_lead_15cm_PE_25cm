...
DetectorRegister<MeshGrid_cuowei, std::string, BambooParameters>
  MeshGrid_cuowei::reg("MeshGrid_cuowei");
...
bool MeshGrid_cuowei::constructMainLV(const BambooParameters&)
{
  ...
  auto wire_pitch =
    parameters.evaluateParameter("wire_pitch");  // 本义网格间距，本代码作为六边形边长使用
...
#else
  auto small_box =
    new G4Box("small_box", wire_diameter / 100, wire_diameter / 100, wire_diameter / 100);  // 基点
  ...
  for (int jl = -n_wires_z - 1; jl <= n_wires_z + 1; ++jl)
  {
    ...
    {
      G4ThreeVector position1 = G4ThreeVector((jp + abs(jl % 2) / 2.0) * sqrt(3) * wire_pitch, 0.,
                                              1.5 * jl * wire_pitch - wire_pitch);
      ...
      G4ThreeVector position2 =
        G4ThreeVector((jp + abs(jl % 2) / 2.0) * sqrt(3) * wire_pitch + sqrt(3) / 4.0 * wire_pitch,
                      0., 1.5 * jl * wire_pitch - 3 / 4.0 * wire_pitch - wire_pitch);
  ...
  auto origin = new G4Box("origin", wire_diameter / 10, wire_diameter / 10,
                          wire_diameter / 10);  // origin为基点
  ...
  auto meshgrid_cuowei =
    new G4UnionSolid(soliname, origin, electrode,
                     transform);  // 并集，transform 定义了 electrode 相对于 origin 的位置和方向。
  ...
  else
  {
    ...
    auto veto_veto_solid =
      new G4ExtrudedSolid(soliname, polygon, 2 * largest_height, offsetA, scaleA, offsetB, scaleB);
...
std::vector<G4TwoVector> MeshGrid_cuowei::GetPolygon(const G4int& sides, const G4double& radius)
{
