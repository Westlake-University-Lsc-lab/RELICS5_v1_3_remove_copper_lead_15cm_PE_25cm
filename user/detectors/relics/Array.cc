...
Array::Array(const std::string& n, const BambooParameters& pars) : BambooDetector(n, pars)
{
...
bool Array::construct(const BambooParameters& global_pars, BambooDetector* parent)
{
  ...
  {
    auto pPMTsR8520 = new PMTsR8520(name, parameters);
  ...
  {
    auto pCuboid = new Cuboid(name, parameters);
    pCuboid->constructMainLV(global_pars);
    CombineLV = pCuboid->getMainLV();
    SensitiveLV = pCuboid->getMainLV();
  }
  else if (objects == "Prism")
  {
    auto pPrism = new Prism(name, parameters);
    pPrism->constructMainLV(global_pars);
    CombineLV = pPrism->getMainLV();
    SensitiveLV = pPrism->getMainLV();
  }
  else if (objects == "Tube")
  {
    auto pTube = new Tube(name, parameters);
    pTube->constructMainLV(global_pars);
    CombineLV = pTube->getMainLV();
    SensitiveLV = pTube->getMainLV();
  }
  else
  {
    return false;
  ...
  if (source_stream >> nObjects)
  {
    for (int il = 0; il < nObjects; ++il)
    {
      ...
      if (source_stream.fail()) throw std::runtime_error("Array: error read data");
      auto mRot = new G4RotationMatrix();
      mRot->rotateZ(-RotZ * deg);
      mRot->rotateY(-RotY * deg);
      new G4PVPlacement(mRot, G4ThreeVector(X * cm, Y * cm, Z * cm), CombineLV, physvol,
