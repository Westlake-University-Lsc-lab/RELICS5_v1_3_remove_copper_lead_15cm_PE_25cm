...
HolesBoard::HolesBoard(const std::string& n, const BambooParameters& pars) : Monoblock(n, pars)
{
...
#endif
  auto box = new G4Box(soliname, width_x / 2, width_x / 2, height);
...
#else
      solid = new G4SubtractionSolid(soliname, solid, box, transform);
#endif
    }
#if G4VERSION_NUMBER >= 1100
    holes->Voxelize();
    solid = new G4SubtractionSolid(soliname, solid, holes);
#endif
  }
  else
  {
