...
DetectorRegister<CylinderOp, std::string, BambooParameters> CylinderOp::reg("CylinderOp");

CylinderOp::CylinderOp(const std::string& n, const BambooParameters& pars) : BambooDetector(n, pars)
{
...
bool CylinderOp::construct(const BambooParameters&, BambooDetector* parent)
{
  ...
  // the ptfe shell
  auto shellSolid =
    new G4Tubs("shellSolid", 0, radius + thickness, height / 2 + thickness, 0, 2 * M_PI);
  mainLV = new G4LogicalVolume(shellSolid, ptfe, "CylinderOpShellLV", 0, 0, 0);
  mainPV = new G4PVPlacement(0, G4ThreeVector(0, 0, 0), mainLV, "CylinderOpShell",
  ...
  auto mat_opsurf = new G4OpticalSurface("ptfe_opsurf", glisur, polished, dielectric_metal);
  ...
  auto xenonSoild = new G4Tubs("xenonSoild", 0, radius, height / 2, 0, 2 * M_PI);
  containerLV = new G4LogicalVolume(xenonSoild, lXe, "CylinderOpLV", 0, 0, 0);
  new G4PVPlacement(0, G4ThreeVector(0, 0, 0), containerLV, "CylinderOp", mainLV, false, 0);
...
void CylinderOp::defineMaterials()
{
  ...
  auto lXe = new G4Material("LXe", z = 54., a = 131.29 * g / mole, density = 3.020 * g / cm3);
  ...
  G4double lxe_energy[] = {
    ...
    7.8 * eV,     7.15 * eV,    6.5 * eV,     6.6 * eV,     6.51224 * eV, 6.42449 * eV,
    6.33673 * eV, 6.24898 * eV, 6.16122 * eV, 6.07347 * eV, 5.98571 * eV, 5.89796 * eV,
    5.8102 * eV,  5.72245 * eV, 5.63469 * eV, 5.54694 * eV, 5.45918 * eV, 5.37143 * eV,
    5.28367 * eV, 5.19592 * eV, 5.10816 * eV, 5.02041 * eV, 4.93265 * eV, 4.8449 * eV,
    4.75714 * eV, 4.66939 * eV, 4.58163 * eV, 4.49388 * eV, 4.40612 * eV, 4.31837 * eV,
    4.23061 * eV, 4.14286 * eV, 4.0551 * eV,  3.96735 * eV, 3.87959 * eV, 3.79184 * eV,
    3.70408 * eV, 3.61633 * eV, 3.52857 * eV, 3.44082 * eV, 3.35306 * eV, 3.26531 * eV,
    3.17755 * eV, 3.0898 * eV,  3.00204 * eV, 2.91429 * eV, 2.82653 * eV, 2.73878 * eV,
    2.65102 * eV, 2.56327 * eV, 2.47551 * eV, 2.38776 * eV, 2.3 * eV};
  const G4int lxenum_scint = 3;
  const G4int lxenum_ceren = 53;
  G4double lxe_scint[] = {1.0, 1.0, 1.0};

  G4double lxe_rind[] = {// 3 + 50
  ...
  ;
  G4double lxe_absl[] = {
    1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm,
    1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm,
    1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm,
    1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm,
    1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm, 1000. * cm,
