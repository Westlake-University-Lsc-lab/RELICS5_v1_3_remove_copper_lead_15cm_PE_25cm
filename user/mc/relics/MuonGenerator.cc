...
GeneratorRegister<MuonGenerator> MuonGenerator::reg("MuonGenerator");
  ...
  : BambooGenerator{pars}, gun{new G4ParticleGun}
{
  ...
  {
    E_low = BambooUtils::evaluate(generatorParameters.getParameter("E_low"));
  }
  if (pmap.find("E_up") != pmap.end())
  {
    E_up = BambooUtils::evaluate(generatorParameters.getParameter("E_up"));
  }
