...
{
  opHitsCollection = new RelicsOpticalHitsCollection(SensitiveDetectorName, collectionName[0]);
  hc->AddHitsCollection(GetCollectionID(0), opHitsCollection);
  eDHitsCollection =
    new PandaXEnergyDepositionHitsCollection(SensitiveDetectorName, collectionName[1]);
...
G4bool RelicsOpticalSD::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
  ...
  if (track->GetParticleDefinition() == G4OpticalPhoton::OpticalPhotonDefinition()) return false;
  ...
  if (parentId != 0)
  {
    auto trackInfo = static_cast<PandaXTrackInformation*>(track->GetUserInformation());
...
G4bool RelicsOpticalSD::ProcessHits_constStep(const G4Step* aStep, G4TouchableHistory*)
{
  auto track = aStep->GetTrack();
  if (track->GetDefinition() != G4OpticalPhoton::OpticalPhotonDefinition()) return false;
  auto pmt_number = aStep->GetPostStepPoint()->GetTouchable()->GetReplicaNumber(2);
  // we need to know that here `2` is just hard code
  // we can use `G4VTouchable::GetVolume` to search the PV name is geometry tree

  auto info = static_cast<RelicsOpticalTrackInformation*>(track->GetUserInformation());
