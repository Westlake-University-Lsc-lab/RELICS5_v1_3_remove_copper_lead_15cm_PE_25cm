...
GeneratorRegister<NeutronGenerator> NeutronGenerator::reg("NeutronGenerator");
  ...
  : BambooGenerator{pars}, gun{new G4ParticleGun}
{
  ...
  {
    shielding_x = BambooUtils::evaluate(generatorParameters.getParameter("shielding_x"));
  }
  else
  {
  ...
  {
    shielding_z = BambooUtils::evaluate(generatorParameters.getParameter("shielding_z"));
  }
  else
  {
...
void NeutronGenerator::NeutronLoad(const std::string& energySpectrumFile,
...
void NeutronGenerator::LoadAngularDistribution(const std::string& angularDistributionFile)
{
  ...
  if (!file.is_open())
  {
    std::cerr << "Error: Failed to open angular distribution file: " << angularDistributionFile
              << std::endl;
    return;
  }
...
void NeutronGenerator::GeneratePrimaries(G4Event* anEvent)
{
  ...
  std::piecewise_constant_distribution<double> disEnergy(energyBins.begin(), energyBins.end(),
                                                         energyValue.begin());
  ...
  // double randomEnergy = G4UniformRand() * (energyBins.back() - energyBins.front()) +
  // energyBins.front(); energy = randomEnergy;

  // // Generate a random direction based on the angular distribution
  // double randomAngle = G4UniformRand() * (angleBins.back() - angleBins.front()) +
  // angleBins.front(); double cosTheta = randomAngle;

  std::piecewise_constant_distribution<double> disAngle(angleBins.begin(), angleBins.end(),
                                                        angularDistribution.begin());
