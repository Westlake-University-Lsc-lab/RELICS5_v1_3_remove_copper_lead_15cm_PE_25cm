...
{
  opHitsCollection = new PandaXOpticalHitsCollection(SensitiveDetectorName, collectionName[0]);
...
G4bool PandaXOpticalSD::ProcessHits_constStep(const G4Step* aStep, G4TouchableHistory*)
{
  auto track = aStep->GetTrack();
  if (track->GetDefinition() != G4OpticalPhoton::OpticalPhotonDefinition()) return false;
  auto pmt_number = aStep->GetPostStepPoint()->GetTouchable()->GetReplicaNumber(1);
