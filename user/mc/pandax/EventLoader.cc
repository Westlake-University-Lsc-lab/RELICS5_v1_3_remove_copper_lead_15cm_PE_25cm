...
EventLoader::EventLoader(const BambooParameters& pars)
  : BambooGenerator{pars},
    messenger{new EventLoaderMessenger(this)},
    source{pars.getParameter("source")},
    source_stream{source}
{
  if (!source_stream.is_open())
    throw std::runtime_error("EventLoader: Cannot open source file " + source);
}
...
void EventLoader::GeneratePrimaries(G4Event* anEvent)
{
  ...
  if (source_stream >> event_number >> n_particles)
  {
    ...
    for (int il = 0; il < n_particles; ++il)
    {
      ...
      if (source_stream.fail()) throw std::runtime_error("EventLoader: error read data");
      auto particle = new G4PrimaryParticle(code, px * GeV, py * GeV, pz * GeV);
...
{
  if (shape == "point") return center;
  if (shape != "cylinder") return center;
  if (radius == 0 || height == 0) return center;

  auto pos = sampleCylinderPosition();
  if (confineVolume == "") return pos;
...
G4bool EventLoader::isSourceConfined(const G4ThreeVector& pos)
{
  ...
  if (name == confineVolume) return true;
  return false;
}
